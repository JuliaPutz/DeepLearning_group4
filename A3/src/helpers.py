...
# define standard values as cornerpoints of image normalization
...
# create a dictionary for easy conversion between semantic mask values and actual labels
id2label = json.load(open(hf_hub_download("segments/sidewalk-semantic", "id2label.json", repo_type="dataset"), "r"))
...
def infer_img(processor, model, image):
    # with a given processor and model, perform semantic segmentation on the given image
    ...
    # process the output to get the segmentation
    seg = processor.post_process_semantic_segmentation(outputs, target_sizes=[image.size[::-1]])[0]
    return seg

def show_overlay(img, seg):
    '''Show the image overlaid with the segmentation'''
...
def denormalize(image):
    # undo the normalization to return to a regular image for viewing
